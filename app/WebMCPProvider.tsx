...
function readActivity(): WebMCPActivity[] {
  try {
    ...
    return Array.isArray(parsed) ? parsed.slice(0, 12) as WebMCPActivity[] : [];
  } catch {
    return [];
  }
}
...
function recordActivity(activity: WebMCPActivity) {
  ...
  window.dispatchEvent(new CustomEvent(WEBMCP_EVENTS.activity, { detail: activity }));
}

function savePreparedPayment(payment: PreparedWebMCPPayment) {
  sessionStorage.setItem(WEBMCP_STORAGE.preparedPayment, JSON.stringify(payment));
  window.dispatchEvent(new CustomEvent(WEBMCP_EVENTS.preparedPayment, { detail: payment }));
}
...
export default function WebMCPProvider() {
  ...
  useEffect(() => {
    ...
    if (!context || typeof context.registerTool !== "function") {
      ...
      window.dispatchEvent(new CustomEvent(WEBMCP_EVENTS.ready, { detail: { supported: false, count: 0 } }));
      return;
    }
    ...
    const tools = createWebMCPTools({
      ...
      savePreparedPayment,
      loadPreparedPayment: () => decodePreparedWebMCPPayment(sessionStorage.getItem(WEBMCP_STORAGE.preparedPayment)),
      recordActivity,
    ...
    void (async () => {
      ...
      root.dataset.webmcp = count === tools.length ? "ready" : count > 0 ? "partial" : "error";
      root.dataset.webmcpCount = String(count);
      window.dispatchEvent(new CustomEvent(WEBMCP_EVENTS.ready, { detail: { supported: true, count } }));
    })();

    return () => {
      lifecycle.abort();
      if (browserWindow.__g402WebMCPLifecycle === lifecycle) delete browserWindow.__g402WebMCPLifecycle;
    };
  }, [router]);
