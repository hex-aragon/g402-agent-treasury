...
import { GnoRpcClient,verifyGnoPayment } from "@/lib/gno";
import { authorize,rateLimit,safeError } from "@/lib/http";
import { appendAudit,claimSettlement,resumeApprovedSettlement,savePayment } from "@/lib/store";
...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    assertSettlementAllowed(payload.network);
    ...
    const record={id:crypto.randomUUID(),paymentId,fingerprint,nonce:requirements.extra.nonce,txHash:null,network:payload.network,payer:payload.payload.payer,payTo:requirements.payTo,asset:requirements.asset,amount:requirements.amount,status:(decision.reason==="approval_required"?"approval_required":"settling") as "approval_required"|"settling",error:null,merchantId:requirements.extra.merchantId,agentId:requirements.extra.agentId,policyId:requirements.extra.policyId,serviceQuoteId:requirements.extra.quoteId,createdAt:new Date().toISOString()};
    ...
    if(!claim.claimed){const e=claim.existing;if(e?.status==="approval_required"&&await resumeApprovedSettlement(paymentId,fingerprint)){}else return NextResponse.json({success:e?.status==="settled",transaction:e?.txHash,network:e?.network||payload.network,replayed:true,errorReason:e?.error||e?.status},{status:e?.status==="settled"?200:409})}
    ...
    const result=await new GnoRpcClient().broadcastTxCommit(payload.payload.signedTx,payload.network);
    await savePayment({...record,txHash:result.transaction||null,status:result.success?"broadcast":"failed",error:result.errorReason||null});
  ...
  }catch(e){const m=e instanceof Error?e.message:"invalid_request";return safeError(m,m==="settlement_disabled"||m==="mainnet_locked"?503:m==="nonce_reused"||m==="idempotency_conflict"?409:400)}
}
